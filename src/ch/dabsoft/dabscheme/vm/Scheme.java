...
public class Scheme {
    ...
    private Compiler compiler = new Compiler(globals);
    private VM vm = new VM(globals);
    ...
    public Token readToken(PushbackReader in) throws IOException {
        ...
        if (state == 2) throw new IllegalStateException("String not closed");
    ...
    public Object read(Token token, PushbackReader in) throws IOException {
        ...
        if (token.type == TokenType.INTEGER) {
            ...
            if (token.type == TokenType.CLOSEPAR) {
                ...
                if (token.type != TokenType.CLOSEPAR) throw new IllegalStateException("Expected )");
                return result;
            }
